...
simulation <- function(cut_off){
  my_simulation_procedure<-function(cut_off){
    ...
    sigma_candidate <- seq(100, 300, by = 50)
        ...
        for(fold in 1:5){
          ...
          for(ppp in 1:length(sigma_candidate)){
            for(qqq in 1:length(lambda_candidate)){
              ...
              for(t in 2:T){
                feature_mat <- train_X_array[, 1:cov_dim[t], t]
                for(i in 1:train_n){
                  for(j in 1:train_n){
              ...
              if(test_n > 1){
                ...
                for(t in 2:T){
                  feature_mat_train <- train_X_array[, 1:cov_dim[t], t]
                  feature_mat_test <- test_X_array[, 1:cov_dim[t], t]
                  for(i in 1:test_n){
                    for(j in 1:train_n){
